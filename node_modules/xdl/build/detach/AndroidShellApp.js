...
let regexFileAsync = (() => {
  var _ref = _asyncToGenerator(function* (regex, replace, filename) {
    let file = yield fs.promise.readFile(filename);
    let fileString = file.toString();
    yield fs.promise.writeFile(filename, fileString.replace(regex, replace));
  });
...
let deleteLinesInFileAsync = (() => {
  var _ref2 = _asyncToGenerator(function* (startRegex, endRegex, filename) {
    ...
    yield fs.promise.writeFile(filename, filteredLines.join('\n'));
  });
...
let copyInitialShellAppFilesAsync = exports.copyInitialShellAppFilesAsync = (() => {
  var _ref4 = _asyncToGenerator(function* (androidSrcPath, shellPath) {
    ...
    yield copyToShellApp('local.properties');
...
let runShellAppModificationsAsync = exports.runShellAppModificationsAsync = (() => {
  var _ref7 = _asyncToGenerator(function* (context) {
    ...
    yield regexFileAsync('VERSION_NAME = null', `VERSION_NAME = "${androidVersion}"`, path.join(shellPath, 'expoview', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'Constants.java'));
    ...
    yield regexFileAsync('INITIAL_URL = null', `INITIAL_URL = "${url}"`, path.join(shellPath, 'expoview', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'Constants.java'));
    if (scheme) {
      yield regexFileAsync('SHELL_APP_SCHEME = null', `SHELL_APP_SCHEME = "${scheme}"`, path.join(shellPath, 'expoview', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'Constants.java'));
    }
    if (shouldShowLoadingView(manifest)) {
      yield regexFileAsync('SHOW_LOADING_VIEW_IN_SHELL_APP = false', 'SHOW_LOADING_VIEW_IN_SHELL_APP = true', path.join(shellPath, 'expoview', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'Constants.java'));
    }
    ...
    if (isFullManifest) {
      ...
      embeddedResponses.add(new EmbeddedResponse("${bundleUrl}", "assets://shell-app.bundle", "application/javascript"));`, path.join(shellPath, 'expoview', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'Constants.java'));
    }

    yield regexFileAsync('RELEASE_CHANNEL = "default"', `RELEASE_CHANNEL = "${releaseChannel}"`, path.join(shellPath, 'expoview', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'Constants.java'));
    ...
    if (iconUrl) {
      ...
      yield saveUrlToPathAsync(iconUrl, path.join(shellPath, 'app', 'src', 'main', 'res', 'mipmap-hdpi', 'ic_launcher.png'));
      ...
      yield saveUrlToPathAsync(iconUrl, path.join(shellPath, 'expoview', 'src', 'main', 'res', 'mipmap-hdpi', 'ic_launcher.png'));
    }
    ...
    if (notificationIconUrl) {
      ...
      yield saveUrlToPathAsync(notificationIconUrl, path.join(shellPath, 'app', 'src', 'main', 'res', 'drawable-hdpi', 'shell_notification_icon.png'));
      ...
      yield saveUrlToPathAsync(notificationIconUrl, path.join(shellPath, 'expoview', 'src', 'main', 'res', 'drawable-hdpi', 'shell_notification_icon.png'));
    }
    ...
    if (backgroundImages && backgroundImages.length > 0) {
      ...
      _.forEach(backgroundImages, (() => {
        var _ref8 = _asyncToGenerator(function* (image) {
  ...
  return function runShellAppModificationsAsync(_x12) {
...
let buildShellAppAsync = (() => {
  var _ref9 = _asyncToGenerator(function* (context) {
    ...
    if (context.build.android) {
      ...
      yield fs.copy(path.join(androidSrcPath, 'debug.keystore'), path.join(shellPath, 'debug.keystore'));
      try {
  ...
  return function buildShellAppAsync(_x14) {
...
exports.updateAndroidShellAppAsync = (() => {
  var _ref3 = _asyncToGenerator(function* (args) {
    ...
    yield deleteLinesInFileAsync(`START\ EMBEDDED\ RESPONSES`, `END\ EMBEDDED\ RESPONSES`, path.join(shellPath, 'expoview', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'Constants.java'));
    ...
    // END EMBEDDED RESPONSES`, path.join(shellPath, 'expoview', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'Constants.java'));
...
exports.createAndroidShellAppAsync = (() => {
  var _ref6 = _asyncToGenerator(function* (args) {
    ...
    if (privateConfigFile) {
      let privateConfigContents = yield fs.promise.readFile(privateConfigFile, 'utf8');
    ...
    let context = (_StandaloneContext || _load_StandaloneContext()).default.createServiceContext(androidSrcPath, null, manifest, privateConfig, configuration, url, releaseChannel, androidBuildConfiguration);

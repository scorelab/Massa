...
let _cleanPropertyListBackupsAsync = (() => {
  var _ref = _asyncToGenerator(function* (context, backupPath) {
    ...
    if (context.type === 'service') {
      yield (_IosPlist || _load_IosPlist()).cleanBackupAsync(backupPath, 'Exponent.entitlements', false);
    }
  });
...
let _preloadManifestAndBundleAsync = (() => {
  var _ref2 = _asyncToGenerator(function* (manifest, supportingDirectory) {
    let bundleUrl = manifest.bundleUrl;
    yield _fs.default.promise.writeFile(`${supportingDirectory}/shell-app-manifest.json`, JSON.stringify(manifest));
    yield (0, (_ExponentTools || _load_ExponentTools()).saveUrlToPathAsync)(bundleUrl, `${supportingDirectory}/shell-app.bundle`);
    return;
  });

  return function _preloadManifestAndBundleAsync(_x3, _x4) {
...
let _configureEntitlementsAsync = (() => {
  var _ref3 = _asyncToGenerator(function* (context) {
    if (context.type === 'user') {
      ...
      const result = (_IosPlist || _load_IosPlist()).modifyAsync(supportingDirectory, 'Exponent.entitlements', function (entitlements) {
  ...
  return function _configureEntitlementsAsync(_x5) {
...
let _configureInfoPlistAsync = (() => {
  ...
  return function _configureInfoPlistAsync(_x6) {
...
let _configureShellPlistAsync = (() => {
  ...
  return function _configureShellPlistAsync(_x7) {
